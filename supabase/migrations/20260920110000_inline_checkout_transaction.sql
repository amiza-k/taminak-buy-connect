-- Do not delegate checkout to checkout_cart_legacy. A few existing databases
-- have a legacy wrapper that resolves back to checkout_cart, which causes a
-- recursive RPC call and PostgreSQL's "stack depth limit exceeded" error.
-- Keep the transaction implementation in the public RPC instead.
...
begin
  ...
  where id = v_cart.organization_id;
